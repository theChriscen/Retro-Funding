...
from constants import DEFAULT_CONFIG, DATA_DIR, RESULTS_DIR
...
def load_yaml_configs(config_path: str) -> Tuple[DataSnapshot, SimulationConfig, AllocationConfig]:
    ...
    data_snapshot = yaml_config['data_snapshot']
    sim_config = yaml_config['simulation']
    alloc_config = yaml_config['allocation']
    return (
        DataSnapshot(
            date=data_snapshot['date'],
            projects_file=data_snapshot['projects_file'],
            metrics_file=data_snapshot['metrics_file']
        ),
        SimulationConfig(
            ...
            normalization=sim_config['normalization']
        ),
        AllocationConfig(
            budget=alloc_config['budget'],
            min_amount_per_project=alloc_config['min_amount_per_project'],
            max_share_per_project=alloc_config['max_share_per_project'],
            max_iterations=alloc_config.get('max_iterations', 50)
        )
    )

def load_csv_data(data_snapshot: DataSnapshot) -> pd.DataFrame:
    """Load and preprocess CSV data."""
    filename = lambda pathname: DATA_DIR / f"{data_snapshot.date}_{pathname}"
...
def save_results(analysis: Dict[str, pd.DataFrame], config_path: str = DEFAULT_CONFIG) -> None:
    ...
    results_df.to_csv(RESULTS_DIR / f"{config_path.stem}.csv")
    print(f"Results saved to {RESULTS_DIR / config_path.stem}.csv")
