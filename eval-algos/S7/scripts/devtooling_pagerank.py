...
def build_graph(
    ...
    source_weights = config['package_dependency'].get('dependency_source_weights', {})
    ...
    dev_data = config['developer_connections']
    event_weights = dev_data['event_weights']
    fanout_exp = dev_data['fanout_exponent']
    half_life_days = dev_data['time_decay']['half_life_days']
    now = datetime.now()
...
def main():
    ...
    # 1) Load data
    data_dir = config['data_files']['data_dir']
    find_path = lambda config_key: os.path.join(data_dir, config['data_files'][config_key])
    df_onchain = pd.read_csv(find_path('onchain_projects'))
    df_devtool = pd.read_csv(find_path('devtooling_projects'))
    df_pkg_links = pd.read_csv(find_path('package_links'))
    df_proj_to_repos = pd.read_csv(find_path('project_to_repositories'))
    df_devs_repos = pd.read_csv(find_path('developers_to_repositories'))
